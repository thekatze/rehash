...
type CreateFormParameters<TFormData extends object> = Parameters<typeof createFormInternal<TFormData>>[0];
...
export const createForm = <TFormData extends object>(params: CreateFormParameters<TFormData>) => {
  ...
  const handleSubmit = (onSubmit: (data: TFormData) => void | Promise<void>) => (event: Event) => {
  ...
  const setErrors = (field: DeepKeyOf<TFormData>, errors: string[]) => setErrorsInternal((e) => {
    const currentErrors = { ...e };
    if (errors.length)
      currentErrors[field] = errors;
    else
  ...
  const registerHandlers = (field: DeepKeyOf<TFormData>, type: HandlerType = "text") => {
    ...
    const onBlur: JSX.EventHandler<HTMLInputElement | HTMLTextAreaElement, FocusEvent> = (event) => {
  ...
  const watch = <TField extends DeepKeyOf<TFormData>>(field: TField) => (): EvaluateKey<TFormData, TField> => deepGetProperty(context().data, field);
  ...
  return { handleSubmit, registerHandlers, errors, setErrors, isDirty, watch, reset };
}
