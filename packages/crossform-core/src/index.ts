...
type FormValidators<TFormData extends object> = Partial<{
  [Field in DeepKeyOf<TFormData>]: FieldValidator<
    EvaluateKey<TFormData, Field>
  >[];
}>;

export type ValidationErrors<TFormData extends object> = Partial<
  Record<DeepKeyOf<TFormData>, string[]>
>;
...
export const createForm = <TFormData extends object>({
  initialData,
  validation,
}: CreateFormProps<TFormData>): FormContext<TFormData> => {
  return {
    data: initialData ?? {},
...
export const isDirty = <TFormData extends object>(
  context: FormContext<TFormData>
) => !deepEquals(context.data, context.initialData);

export const reset = <TFormData extends object>(
  context: FormContext<TFormData>,
  data?: Partial<TFormData>
) => {
...
export const validateField = <TFormData extends object>(
  context: FormContext<TFormData>,
  field: DeepKeyOf<TFormData>
) => {
  return (
    context.validation?.[field]
      ?.map((validator) => validator(deepGetProperty(context.data, field)))
      .filter((result): result is string => Boolean(result)) ?? []
  );
};

export const validateForm = <TFormData extends object>(
  context: FormContext<TFormData>
