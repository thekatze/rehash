...
const OnboardingNavigation: VoidComponent<{
  setStep: Setter<number>;
  continueText?: string;
}> = (props) => {
  const [t] = useI18n();
  return (
    <div class="flex gap-2 ml-auto mt-4">
      <Button variant="ghost" onClick={() => props.setStep((s) => s - 1)}>
        {t("onboarding.go_back")}
      </Button>
      <Button variant="primary" onClick={() => props.setStep((s) => s + 1)}>
...
const OnboardingWelcome: OnboardingStep = (props) => {
  ...
  const importFile = (text: string) => {
    ...
    set(STORE_KEY, JSON.parse(text)).then(() => window.location.assign("/"));
  };
...
const OnboardingLanguage: OnboardingStep = (props) => {
  ...
  return (
    <>
      ...
      <Subheading>
        {t("onboarding.language.set_your_preferred_language")}
      </Subheading>
      <Paragraph>
        {t("onboarding.language.set_your_preferred_language_text")}
      </Paragraph>
      <LanguageSelect />
...
const OnboardingWhatIsRehash: OnboardingStep = (props) => {
  ...
  const RowSelectButton: VoidComponent<{
    value: string;
    ...
    <td>
      <Button
        class="w-full"
        variant={
          selection()[props.field] === props.value ? "primary" : "secondary"
        }
        onClick={() => {
          setSelection((s) => ({ ...s, [props.field]: props.value }));
          setClicked(true);
        }}
      >
        {props.value}
  ...
  return (
    <>
      ...
      <Paragraph>
        {t("onboarding.what_is_rehash.how_does_it_work_text")}
      </Paragraph>
      <Paragraph>
        <table class="w-full">
          <tbody>
      ...
      <Paragraph>
        {t("onboarding.what_is_rehash.cross_device_generation")}
      </Paragraph>
      <Paragraph>{t("onboarding.what_is_rehash.next_steps")}</Paragraph>
      <OnboardingNavigation
        setStep={props.setStep}
        continueText={t("onboarding.create_your_vault.create_vault")}
      />
    </>
  );
};
    ...
    <>
      ...
      <Paragraph>
        <Toggle
          ...
          onChange={() => setEncryptVault(!encryptVault())}
        />
      </Paragraph>
      <OnboardingNavigation setStep={props.setStep} />
    </>
...
const [generatorSettings, setGeneratorSettings] =
  createSignal<GeneratorOptions>(recommendedDifficulty);

const OnboardingVaultSettings: OnboardingStep = (props) => {
  const [t] = useI18n();
  const [form] = createForm<StoreEntry>({
    initialValues: { generatorOptions: generatorSettings() },
  });
  ...
  return (
    <>
      <Heading>{t("onboarding.vault_settings.vault_settings")}</Heading>
      <Subheading>
        {t("onboarding.vault_settings.set_generation_difficulty")}
      </Subheading>
      <Paragraph>
      ...
      <Stack direction="column" class="mt-8 mb-4 gap-4 items-center">
        ...
        <pre class="flex-1 relative">
          <Show
            ...
            fallback={
              <pre>
                {t("onboarding.vault_settings.took_seconds", {
                  seconds: "X.XXX",
                })}
              </pre>
            }
...
const OnboardingCreateVault: OnboardingStep = (props) => {
  ...
  return (
    <>
      ...
      <Paragraph>
        {t("onboarding.create_your_vault.set_password_text")}
      </Paragraph>
